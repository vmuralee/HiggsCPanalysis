...
int main(int argc, char * argv[]) {
  ...
  tree->Branch("idisoweight_1", &idisoweight_1, "idisoweight_1/F");
  ...
  tree->Branch("os", &os, "os/F");
  tree->Branch("Prompt_pT", &Prompt_pT, "Prompt_pT/D");
  tree->Branch("againstMuonTight3_2", &againstMuonTight3_2, "againstMuonTight3_2/F");
  tree->Branch("againstElectronLooseMVA6_1",&againstElectronLooseMVA6_1,"againstElectronLooseMVA6_1/F");
  tree->Branch("againstElectronLooseMVA6_2",&againstElectronLooseMVA6_2,"againstElectronLooseMVA6_2/F");
  tree->Branch("againstElectronMediumMVA6_1",&againstElectronMediumMVA6_1,"againstElectronMediumMVA6_1/F");
  tree->Branch("againstElectronMediumMVA6_2",&againstElectronMediumMVA6_2,"againstElectronMediumMVA6_2/F");
  tree->Branch("againstElectronTightMVA6_1",&againstElectronTightMVA6_1,"againstElectronTightMVA6_1/F");
  ...
  tree->Branch("pt_lead", &pt_lead, "pt_lead/F");
  tree->Branch("phi_tau1", &phi_tau1, "phi_tau1/F");
  tree->Branch("eta_lead", &eta_lead, "eta_lead/F");
  ...
  tree->Branch("pt_trail", &pt_trail, "pt_trail/F");
  tree->Branch("phi_tau2", &phi_tau2, "phi_tau2/F");
  tree->Branch("eta_trail", &eta_trail, "eta_trail/F");
  ...
  tree->Branch("pass_iso", &pass_iso, "pass_iso/O");
  tree->Branch("fail_iso", &fail_iso, "fail_iso/O");
  ...
  tree->Branch("gen_noutgoing", &gen_noutgoing, "gen_noutgoing/I");
  tree->Branch("gen_noutgoing_NLO", &gen_noutgoing_NLO, "gen_noutgoing_NLO/I");
 ...
 // end of Sych tree
  ...
  TauTriggerSFs2017 * tauTriggerSF = new TauTriggerSFs2017(cmsswBase+"/src/TauAnalysisTools/TauTriggerSFs/data/tauTriggerEfficiencies2017.root","ditau","2017","medium","MVAv2"); 
  // Tau Scale factors
  ...
  for (int iF=0; iF<nTotalFiles; ++iF) {
	...
	//if (analysisTree.tau_byMediumIsolationMVArun2017v2DBoldDMwLT2017[it] < 0.5) continue;//*applied medium criteria after selecting index pair
	...
	os=analysisTree.tau_charge[tauIndex_1]*analysisTree.tau_charge[tauIndex_2];
	/****QCD Weight********************/
	int tag=-1;int probe=-1;pass_iso=false;fail_iso=false;
	if(analysisTree.tau_byMediumIsolationMVArun2017v2DBoldDMwLT2017[tauIndex_1]>0.5&&analysisTree.tau_byVLooseIsolationMVArun2017v2DBoldDMwLT2017[tauIndex_2]>0.5){
	  tag = tauIndex_1;
	  probe = tauIndex_2;
	}
	if(analysisTree.tau_byMediumIsolationMVArun2017v2DBoldDMwLT2017[tauIndex_2]>0.5&&analysisTree.tau_byVLooseIsolationMVArun2017v2DBoldDMwLT2017[tauIndex_1]>0.5){
	  probe = tauIndex_1;
	  tag = tauIndex_2;
	}
	if(tag>-1&&probe>-1){
	  if(analysisTree.tau_byMediumIsolationMVArun2017v2DBoldDMwLT2017[tag]>0.5&&analysisTree.tau_byMediumIsolationMVArun2017v2DBoldDMwLT2017[probe]>0.5)
	    pass_iso=true;
	  if(analysisTree.tau_byMediumIsolationMVArun2017v2DBoldDMwLT2017[tag]>0.5&&analysisTree.tau_byMediumIsolationMVArun2017v2DBoldDMwLT2017[probe]<0.5)
	    fail_iso=true;
	}
	...
	againstElectronLooseMVA6_2 = analysisTree.tau_againstElectronLooseMVA6[tauIndex_2];
	againstElectronMediumMVA6_2 = analysisTree.tau_againstElectronMediumMVA6[tauIndex_2];
	againstElectronTightMVA6_2 = analysisTree.tau_againstElectronTightMVA6[tauIndex_2];
	...
	againstElectronLooseMVA6_1 = analysisTree.tau_againstElectronLooseMVA6[tauIndex_1];
	againstElectronMediumMVA6_1 = analysisTree.tau_againstElectronMediumMVA6[tauIndex_1];
	againstElectronTightMVA6_1 = analysisTree.tau_againstElectronTightMVA6[tauIndex_1];
	...
	float minDR_2 = 0.2;
	if(isDY||isW){
	  gen_noutgoing=analysisTree.genparticles_noutgoing;
	...
	if(!(tauPass1&&tauPass2))continue;
	//TriggerSF
	if(!isData){
	  trigweight_1=tauTriggerSF->getTriggerScaleFactor(analysisTree.tau_pt[tauIndex_1],analysisTree.tau_eta[tauIndex_1],analysisTree.tau_phi[tauIndex_1],analysisTree.tau_decayMode[tauIndex_1]);
	  trigweight_2=tauTriggerSF->getTriggerScaleFactor(analysisTree.tau_pt[tauIndex_2],analysisTree.tau_eta[tauIndex_2],analysisTree.tau_phi[tauIndex_2],analysisTree.tau_decayMode[tauIndex_2]);
	  trigweight = trigweight_1*trigweight_2;
	  if(isDY){
	  if(gen_match_1==5)
	    idisoweight_1=tau_id_sf;
	  if(gen_match_2==5)
	    idisoweight_2=tau_id_sf;
	  if(gen_match_1==3)
	    idisoweight_1=elec_id_sf(analysisTree.tau_eta[tauIndex_1]);
	  if(gen_match_2==3)
	    idisoweight_2=elec_id_sf(analysisTree.tau_eta[tauIndex_2]);
	  if(gen_match_1==4)
	    idisoweight_1=muon_id_sf(analysisTree.tau_eta[tauIndex_1]);
	  if(gen_match_2==4)
	    idisoweight_2=muon_id_sf(analysisTree.tau_eta[tauIndex_2]);

	  }
	  
	}
	effweight=trigweight*idisoweight_1*idisoweight_2;
	//cout<<effweight<<endl;
	...
	iso_plus = analysisTree.tau_byMediumIsolationMVArun2v1DBoldDMwLT[tauIndex_1];
	...
	iso_neg = analysisTree.tau_byMediumIsolationMVArun2v1DBoldDMwLT[tauIndex_2];
	m_neg = analysisTree.tau_mass[tauIndex_2];
	
	pt_plus>pt_neg?pt_lead=pt_plus:pt_neg;
	pt_plus<pt_neg?pt_trail=pt_plus:pt_neg;
	pt_plus>pt_neg?eta_lead=eta_plus:eta_neg;
	pt_plus<pt_neg?eta_trail=eta_plus:eta_neg;
